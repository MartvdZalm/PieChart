<template>
  <svg class="donutchart" :width="width" :height="height">
    <g :transform="`translate(${width / 2}, ${height / 2})`">
      ...
      <g v-for="(slice, index) in slices" :key="index" class="piece" @mouseover="showValues(slice.value, slice.date)" @mouseout="hideValues">
        <path :d="createArc(outerRadius, slice.startAngle, slice.endAngle)" :fill="slice.color"></path>
        <path :d="createArc(outerRadius, slice.startAngle, slice.endAngle)" :fill="slice.color" class="hover-piece"></path>
      </g>
    </g>
    <text text-anchor="middle" :style="font" :dy="160" :dx="80">{{ title }}</text>
  </svg>
</template>
...
export default {
  ...
  methods: {
    ...
    createArc(outerRadius, startAngle, endAngle) {
